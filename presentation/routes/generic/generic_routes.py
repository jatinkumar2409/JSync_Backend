...
async def refresh_token_route(request : Request , refresh_token : str = Cookie(None)):
...
async def get_session(request : Request):
    try:
      ...
      user = verify_token(token=ref_token ,token_type="refresh")
      return user
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500 , detail=str(e))
...
async def refresh_token(request : Request):
    try:
        ...
        if not ref_token:
          raise HTTPException(status_code=404, detail="session not found")
        payload = verify_token(token=ref_token , token_type="refresh")
